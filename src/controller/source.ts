...
export default {
  ...
  async put(req: Request, res: Response) {
    const result = validation.source.update.validate({ ...req.body, ...req.params });
    if (result.error) {
      return responseBuilder.badRequest(res, req.params, result.error.message);
    }
    try {
      let { id, source } = await Joi.attempt(result.value, validation.source.update);
      const updatedSource = await service.CRUD.updateById('Source', source, id, [], '');
      if (!updatedSource) {
        return responseBuilder.notFound(res, '', 'source notFound!');
      }
