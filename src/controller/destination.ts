...
export default {
  ...
  async getAll(req: Request, res: Response) {
    try {
      const destinationes = await Service.CRUD.find("Destination", {}, [], "", "");
      return responseBuilder.success(res, destinationes, "");
    } catch (err) {
      console.log("✖ err from catch of controller : ", err);
      //   console.log(chalk.red("✖ err from catch of controller : "),err );
      return responseBuilder.internalErr(res);
  ...
  async put(req: Request, res: Response) {
    const result = validation.destination.update.validate({
      ...req.body,
      ...req.params,
    });
    if (result.error) {
    ...
    try {
      //?=======
      let { id, destination } = await Joi.attempt(
        result.value,
        validation.destination.update
      );
